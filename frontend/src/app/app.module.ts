...
@NgModule({
  ...
  imports: [
    ...
    // MSAL for Entra ID authentication (redirect flow — popup is blocked by COOP policy)
    MsalModule.forRoot(
      msalInstance,
      {
      ...
      {
        // We don't use MSAL interceptor for our own API (we use our own Prevello JWT).
        // Empty map means MSAL won't auto-attach tokens to any requests.
        interactionType: InteractionType.Redirect,
