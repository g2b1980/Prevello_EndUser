...
export class TopNavComponent implements OnInit {
    ...
    setTimeout(() => {
      ...
      window.addEventListener('resize', () => {
				if (window.innerWidth >= 768) {
					...
					this.handleTopNavMenuFocus();
				}
				this.handleTopNavDrag('.app-top-nav');
			});
	...
	handleTopNavMenuFocus() {
		...
		const viewNav = document.querySelector('.app-top-nav') as HTMLElement | null;
		...
		const elms = Array.from(document.querySelectorAll('.app-top-nav .menu > .menu-item')) as HTMLElement[];
		if (elms.length > 0) {
			let found = false;
	...
	handleTopNavDrag(containerClassName: string) {
	...
	handleTopNavControlAction(event: MouseEvent, direction: string) {
		...
		const containerWidth = (document.querySelector('.app-top-nav') as HTMLElement).clientWidth - (document.querySelector('.app-top-nav') as HTMLElement).clientHeight * 2;
		let totalWidth = 0;
	...
	handleTopNavToggle(menus: HTMLElement[], forMobile = false) {
  ...
  handleTopNavSubMenu() {
    ...
    this.handleTopNavToggle(menus, true);
    ...
    this.handleTopNavToggle(submenusLvl1);
    ...
    this.handleTopNavToggle(submenusLvl2);
  }
}
